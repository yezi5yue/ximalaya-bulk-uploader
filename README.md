...
- **不删除重排顺序**：发布后发现顺序乱了，可用 `reorder_album.py` 直接调用创作中心内部 `changeOrder` 接口，按文件名重新排序，无需删除重发。
...
如果分多批补发导致顺序乱了，可以用 `reorder_album.py` 直接调用喜马拉雅创作中心的内部 `changeOrder` 接口，按文件名重新排序，**不需要删除任何声音**。
...
1. 先把不含"复习"的完整课声音按章节/讲次排好。
2. 再把含"复习"的复习音频按章节/复习编号排好。
3. 最终顺序：完整课在前，复习音频在后，均按章节升序。

脚本会：

- 读取当前真实在线顺序与 track ID。
- 备份原顺序到 `/tmp/album_order_backup.json`。
- 用插入排序方式逐个调用 `changeOrder` 移动声音。
- 两次移动之间加入随机延迟（默认 `8 + 0~8s`）。
- 完成后重新拉取在线顺序，确认已与目标顺序一致才退出。

> ⚠️ 该脚本会直接修改线上专辑顺序，请先确认备份文件存在后再运行。
