...
## [v1.7.4] — 2026-09-16（修复：专辑预检漏掉分页专辑，误报 "Album not found"）

### 问题
上传专辑「作文」(121805384) 时，批次在正式发布前就被中止：
```
Pre-checking that the album exists in your account ...
❌ Album not found: 作文
```
专辑在账号里确实存在，只是没有出现在选择器首屏。

### 根因
专辑选择器是**分页**的（每次只渲染 ~10 个）。v1.7.2 只修好了实际选专辑的
`_select_album()`（滚动加载 + `:text-is()` 精确匹配），但批次开始前的预检
`check_album_exists()` 仍是旧逻辑——只读取首屏 `div.scroll-item-content-252FXLKk`
的文本做子串判断。藏在后面分页的专辑（如「作文」）因此被误判为"不存在"，
批次在传第 1 条之前就终止。

### 修复
- 新增共用函数 `_ensure_album_loaded(page, album_name, max_scrolls=25)`：
  循环点击「加载更多」或滚动最近的可滚动祖先，直到精确标题出现。
- `check_album_exists()` 与 `_select_album()` **共用同一个分页加载函数**，
  预检与实际选专辑不会再出现判断不一致（预检说没有、实际能选到，或反之）。
- `check_album_exists()` 保留子串扫描作为兜底（标记结构变化时不至于直接失败）。

### 影响
分页深处的专辑现在能正常通过预检。已用「作文」(121805384) 实测：预检通过，
10/10 全部发布成功，首条专辑 ID 熔断校验一致。

---

## [v1.7.3] — 2026-08-29（新增：--custom-order-file 自定义发布顺序 + 删除工具 delete_tracks_by_title.py）

### 新增
- `uploader.py` → 新增 CLI 参数 `--custom-order-file <path>`：
  - 从文本文件读取音频 basename（每行一个，不带扩展名，`#` 开头为注释）；
  - 文件里列出的音频按该顺序发布；未列出的音频接在后面，沿用 `--order` 的排序；
  - 适用于"按目录/大纲顺序发布"，而不受文件名排序限制。
- 新增 `delete_tracks_by_title.py`：按标题批量删除指定专辑中的音频（默认 dry-run，`--commit` 生效），用于重排前清空间或删除误传音频。

### 使用场景
- 本次上传「初中历史课程标准」：目录顺序为 中国古代史 → 中国近代史 → 中国现代史 → 世界古代史 → 世界近代史 → 世界现代史 → 学业质量描述；每个板块内部按 概要 → 内容要求 → 教学提示 → 学业要求 排列。用 `--custom-order-file` 精确实现该顺序。

### 已知限制
- `publish_manifest.jsonl` 以标题为唯一键，跨专辑上传同标题音频时会被误跳过。当前 workaround：删除目标专辑里的旧条目后用 `--no-resume` 重新全量上传。
